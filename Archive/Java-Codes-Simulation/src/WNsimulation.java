...
class Tool
{
	...
	public void Toool() throws Exception
	{
		...
		System.out.print("Call off-set time : ");
		calloffset=Integer.parseInt(dis.readLine());
		//System.out.print("Enter Simulation time in minutes: ");
		si_time=2000;//Integer.parseInt(dis.readLine());
...
// Request Allocation Check					
			...
			if(inserv_req.size()<fl_size && local_time>=(REQMATRIX[((Integer)unatt_req.get(0)).intValue()-1][3]-calloffset))
			{
...
// Gettting the Request Details	
	...
	public static void printresults()	
	{
		...
		int tot_nonrev_path=0,tot_car_wait_time=0,div_factor=0, closing_cost=0;
		int car_wait_count=0;
		...
		for(int i=0; i<numreq;i++)
		{
			...
			if (car_sel_req[i][1]!=10000)
			{
				...
				if(car_sel_req[i][5]!=0)
				{	
					//System.out.println(car_sel_req[i][5]);	
				
					//System.out.print("REVENUE GENERATED IN THIS REQUEST is Rs..: ");	
					nonrev_path=SPATHD[car_sel_req[i][6]][car_sel_req[i][7]]+ SPATHD[car_sel_req[i][7]][car_sel_req[i][8]];
					...
					rev_tot=rev_tot+car_sel_req[i][9];
					//System.out.println("Dead_mileage in this request : "+dead_mileage);
				}
				else 
				{
					//System.out.println(car_sel_req[i][5]);
					//System.out.println("Request still in progress.....");		
				}
				//System.out.println();	
		...
		for(int p=0;p<fl_size;p++)
		{
			closing_cost+=(SPATHD[CARMATRIX[p][1]][0]*nonrev_cost)	;
		}
		
				//System.out.print("Closing Cost : "+ closing_cost);	
				System.out.print("TOTAL REVENUE GENERATED (Not closed) :	");	
				System.out.println(rev_tot);
				System.out.print("TOTAL REVENUE AFTER closing : ");	
				System.out.println(rev_tot-closing_cost);	
				System.out.print("AVERAGE CAR WAITING TIME  : ");	
				System.out.println(tot_car_wait_time/car_wait_count);
				System.out.print("AVERAGE DEAD MILEAGE  : ");	
...
class Performance extends Tool
{
	...
	{
		System.out.println("% Rejected due to fleet constraint with fleet size "+fl_size+" is : "+((num_rejected_fleet*100)/numreq));	
	}
...
//		System.out.println("Number of Calls late serviced "+fl_size+" is : "+late_count);		
        System.out.println("% Calls late serviced with fleet size"+fl_size+" is : "+((late_count*100)/numreq));		
     }
}	