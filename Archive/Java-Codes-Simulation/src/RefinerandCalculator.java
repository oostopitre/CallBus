...
public class RefinerandCalculator
{
	...
	public static void createFiles() throws IOException
	{
		...
		for(int i = 0; i < 60; i++)
		{
			for(int j = 0; j < 60; j++)
			{
				BufferedWriter writer = new BufferedWriter(new FileWriter("e:/Project/SlottedData/" + i + "_" + j +".txt"));
	...
	public static void processInput(LinkedList input, LinkedList index) throws IOException
	{
		...
		for(int i=0; i<input.size(); i++)
		{
			...
			reader = new BufferedReader(new FileReader("e:/Project/SlottedData/" + node_origin + "_" + node_dest + ".txt"));
			int k = 0;
			while( (line = reader.readLine()) != null)
			{
				entry = (line.split("\\t"));
			...
			writer = new BufferedWriter(new FileWriter("e:/Project/SlottedData/" + node_origin + "_" + node_dest +".txt"));
			for(int x = 0; x < num; x++)
			{
